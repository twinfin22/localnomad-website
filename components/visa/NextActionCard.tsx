...
export function NextActionCard({ action, className }: NextActionCardProps) {
  ...
  const CardContent = (
    <div className={cn(
      "group vk-card p-6 transition-all duration-300 cursor-pointer",
      action.link && "vk-card-hover",
      className
    )}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3 className="text-lg font-semibold text-[#F8FAFC] mb-1 group-hover:text-cyan-400 transition-colors">
            {action.title}
          </h3>
          ...
          {action.link && (
            <div className="flex items-center gap-1 mt-3 text-sm text-cyan-400 font-medium">
...
export function NextActionsList({ actions, className }: NextActionsListProps) {
  if (actions.length === 0) {
    return (
      <div className={cn("vk-card p-6 text-center", className)}>
        <CheckCircle className="w-12 h-12 text-emerald-400 mx-auto mb-3" />
        <h3 className="text-lg font-semibold text-[#F8FAFC] mb-1">All Done!</h3>
        <p className="text-sm text-[#94A3B8]">No pending actions at this time.</p>
      </div>
    );
  }
