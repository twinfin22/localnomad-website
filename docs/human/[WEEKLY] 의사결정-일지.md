...
## 2026-03-03 (확장 아키텍처 결정)

### 데이터 디렉토리 구조 통일: `{country}/{locale}/{type}.json`

- **맥락**: 한국은 `data/visas/{locale}/`, 대만은 `data/visas/tw/{locale}/`로 패턴 불일치. 일본/중국 추가 시 혼란 가중
- **선택지**: (A) 전부 `data/visas/{country}/{locale}/{type}.json`으로 통일 (B) 현행 유지, 일본/중국만 국가 접두사 추가
- **결정**: A — 전부 통일
- **이유**: B는 한국만 예외로 남아서 로더 코드에 분기가 필요. A로 하면 모든 국가가 같은 패턴이라 로더가 단순해짐.

### 타입 로더: if/else 유지

- **맥락**: 4개국으로 확장 시 visa-data.ts의 국가별 타입 분기를 제네릭 로더로 리팩토링할지
- **선택지**: (A) 제네릭 로더 — 설정 파일 기반 (B) if/else 유지
- **결정**: B — if/else 유지
- **이유**: 4~6개국 수준에서 if/else가 읽기 쉽고 디버깅 쉬움. 제네릭은 10개국 이상일 때 필요. 추상화는 "반복의 고통"이 실제로 느껴질 때 도입.

### 동남아(SEA): Country 타입으로 취급 + 비교 전용

- **맥락**: 동남아는 개별 비자 상세 페이지 없이 "디지털 노마드 비자 비교"만 제공
- **선택지**: (A) 독립 라우트 `/compare/digital-nomad-asia` (B) `/southeast-asia`를 Country로 취급
- **결정**: B — `southeast-asia`를 Country 타입에 포함
- **이유**: 국가 페이지 자체가 "Compare DNV in SEA" 역할. `AVAILABLE_VISAS['southeast-asia']`는 빈 배열, 비교 데이터는 별도 JSON. 나중에 태국 등 개별 승격 시 분리 가능.

### SEA 비교 데이터: 별도 comparison JSON

- **맥락**: SEA 디지털 노마드 비자 비교 데이터를 어디에 저장할지
- **선택지**: (A) 개별 visa JSON — 각 나라별 파일 생성 후 비교 페이지에서 합치기 (B) 별도 comparison JSON — `data/comparisons/sea-digital-nomad.json`
- **결정**: B — 별도 comparison JSON
- **이유**: 지금은 비교 페이지만 있어서 풀 JSON은 과잉. 빠르게 구현 가능. 나중에 개별 나라 승격 시 마이그레이션.

### 확장 국가 언어 우선순위

- **맥락**: 일본/중국/SEA 추가 시 각 국가별 지원 언어 결정
- **결정**: 초기에는 전부 영어(en)만. 이후 확장 우선순위:
  - 🇯🇵 일본: zh-cn → vi → en → ko (중국인 84만, 베트남인 60만이 일본 비자 주 수요)
  - 🇨🇳 중국: en → ja → ko (한국인·일본인·영어권이 주 수요. 베트남어 불필요)
  - 🌏 SEA: en만 (디지털 노마드 시장은 영어 지배적)
- **이유**: 각 국가 체류 외국인 통계 기반. 일본 내 중국 국적자 84만명으로 압도적 1위.
- **📘 배경지식**: 일본어로 일본 비자 정보가 필요 없는 이유 — 일본인은 자국 비자를 신청할 일이 없음. 중국어로 중국 비자 정보도 마찬가지.

### 법률 디스클레이머: 국가별 설정 기반 확장

- **맥락**: 현재 VisaDisclaimer가 korea/taiwan을 if/else로 분기. 4개국+SEA로 확장 필요
- **결정**: 설정 기반으로 리팩토링. 국가별 법률명, 벌칙, 면책 문구를 설정 객체에 정의
- **법률 조사 결과**:
  - 🇯🇵 일본: 행정서사법 §19/§23 — 무자격 비자 상담/대행 금지, 1년 이하 징역 + 100만엔 벌금
  - 🇨🇳 중국: 단일 법률 없으나 무자격 서비스 행정 위반. "咨询" 용어 사용 금지
  - 🇹🇭 태국: Immigration Act B.E. 2522 — 무자격 이민 서비스 금지
  - 🇮🇩 인도네시아: Law No. 6/2011 — IDR 5억+ 벌금
  - 🇲🇾 말레이시아: Immigration Act 1959/63
- **공통 원칙**: 모든 국가에서 "정보 표시 OK, 상담/추천/적격성 판단 NEVER" 동일
- **조사 상세**: `docs/agent/reference/research-expansion-legal-disclaimers.md` (생성 예정)
...
- **맥락**: 4개국 디지털 노마드 가이드를 웹사이트에 블로그/아티클로 추가하기 위한 인프라 필요
- **선택지**:
  1. URL 구조: (A) 플랫 `/blog/slug` (B) 카테고리 포함 `/blog/guides/slug` (C) 국가 포함 `/blog/korea/slug`
  2. MDX 처리: (A) next-mdx-remote (B) Contentlayer (C) next-mdx-remote + Zod 검증
  3. 카테고리: (A) 2개 최소 (B) 4개 중간 (C) 6개 풀스케일
- **결정**: URL=B, MDX=C, 카테고리=C
- **이유**:
  - URL B: `/blog/guides/`로 카테고리별 모아보기가 자연스러움
  - MDX C: Contentlayer(B)는 유지보수 불안정 리스크. Zod 직접 검증으로 같은 타입 안전성을 외부 의존성 없이 확보
  - 카테고리 C: 블로그에 가이드 외에도 뉴스, 노마드 인터뷰 등 다양한 콘텐츠 계획
- **📘 배경지식**: Zod는 데이터 형식 검증 라이브러리. frontmatter(글 메타데이터)에 오타나 누락이 있으면 빌드 시점에 에러를 잡아줌
- **프롬프트**: `docs/agent/prompts/phase-4-blog-infrastructure-prompt.md`

### 온보딩 UX 리디자인

- **맥락**: 기존 온보딩은 "비자 선택 필수 → 만료일 → I don't have a visa yet 체크박스" 순서. 비자가 없는 사용자(대부분)에게 비직관적
- **선택지**:
  - (A) 목표 먼저 — 관심 비자 선택 → "현재 비자 있으세요?" (없어요가 디폴트) → 대시보드
  - (B) 한 화면 — 관심 비자 + 현재 비자를 한 화면에 (모바일에서 빽빽할 수 있음)
  - (C) 질문형 분기 — 사용자 의도별 3갈래 (알아보는 중 / 갱신 / 전환). 직관적이나 구현 복잡
- **결정**: A — 목표 먼저
- **이유**: 단계가 적고 모바일 UX 좋음. 갱신/전환 구분이 목표 비자 vs 현재 비자 비교로 자동 해결됨
- **후속 결정 완료**: DB 스키마 → Option 1 (기존 테이블에 컬럼 추가)
  - `visa_type` → `goal_visa_type` (목표 비자, 항상 존재)
  - `current_visa_type` 추가 (현재 비자, null = 비자 없음이 디폴트)
  - `expiry_date` → `current_expiry_date`
  - 이유: 가장 단순한 마이그레이션. 한 행 조회로 목표+현재 정보 모두 확인 가능
- **프롬프트**: `docs/agent/prompts/onboarding-redesign-prompt.md`

### 홈페이지 국가 확장

- **맥락**: 홈페이지 Hero에 한국/대만만 표시. 일본/중국/SEA 비자 데이터 및 라우트는 이미 준재
- **결정**: Phase 4 프롬프트 Step 0으로 포함. Hero를 grid 레이아웃으로 변경, 5개 CountryCard 표시
- **이유**: 백엔드는 5개국 준비 완료인데 프론트엔드만 2개국 하드코딩이라 불일치 상태

### 비자 상세 페이지 UI 리디자인

- **맥락**: 현재 비자 상세 페이지가 7개 아코디언을 같은 깊이로 나열. 정보 과부하(overwhelming). "이 비자가 나한테 맞는지" 빠른 판단 불가
- **선택지**:
  - (A) 3단계 계층 — Quick Verdict → 탭(자격/서류/절차) → 참고(FAQ/팁/출처). 아코디언을 탭으로 대체
  - (B) 요약 카드 + 아코디언 유지 — 아코디언 위에 3줄 요약 추가. 기존 코드 변경 최소
  - (C) 2페이지 분리 — Overview 페이지 + Full Guide 페이지. SEO 유리하나 구현 가장 큼
- **결정**: A — 3단계 계층 + 탭
- **이유**: 정보 과부하를 가장 효과적으로 해소. 사용자 여정(판단→준비→실행)과 구조가 일치
- **추가 결정**:
  - JSON 데이터에 `priority: "essential" | "detail"` 필드 추가하여 계층 제어
  - F-2를 파일럿으로 먼저 구현, Gen 확인 후 나머지 비자에 적용
- **프롬프트**: `docs/agent/prompts/visa-ui-redesign-prompt.md`
