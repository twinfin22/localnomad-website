...
│       │                    가능해짐 (= hydration)             │
│       │                                                      │
│  ⑦ useEffect 실행         localStorage 읽기, 날짜 계산 등      │
│                            ← ✅ 2025-02-17 flickering 수정됨  │
└─────────────────────────────────────────────────────────────┘
```
...
브라우저가 HTML 수신
    ...
    │
    └─ useEffect 실행: ← ✅ flickering 수정 완료 (2025-02-17)
        ├─ useIsMobile: 삭제됨 (dead code)
        ├─ DDayCounter: Server Component로 전환 → useEffect 불필요
        ├─ VisaJourneyPage: 배너 초기값 반전 (false→true) → 깜빡임 제거
        ├─ ChecklistStep: 중복 hash useEffect 삭제
        ├─ SocialProofBar: new Date() → useState+useEffect (hydration-safe)
        ├─ DDayPanel: new Date() → useState+useEffect (hydration-safe)
        └─ OnboardingResults: new Date() → useState+useEffect (hydration-safe)
```
...
┌──────────────────────────┐
│   브라우저 전용 데이터      │    ← 서버 접근 불가 (flickering 수정 완료)
│                           │
│  localStorage             │    배너 닫힘 여부, 비로그인 진행상황
│  new Date()               │    D-Day 계산 (SocialProofBar, DDayPanel 등)
│  navigator                │    브라우저/OS 정보
│                           │
│  ※ window.innerWidth      │    삭제됨 (useIsMobile 제거)
│  ※ location.hash          │    삭제됨 (ChecklistStep hash 제거)
│                           │
└──────────────────────────┘
```
...
## 9. 삭제된 컴포넌트 (2025-02-17)

> 아래 파일들은 dead code 또는 사용하지 않는 UI 컴포넌트로 삭제됨.

| 파일 | 삭제 이유 |
|------|----------|
| `hooks/use-mobile.ts` | import 0건 — dead code |
| `components/ui/use-mobile.tsx` | import 0건 — dead code |
| `components/ui/sidebar.tsx` | import 0건 — dead code |
| `components/ui/calendar.tsx` | 사용처 없음 |
| `components/ui/carousel.tsx` | 사용처 없음 |
| `components/ui/chart.tsx` | 사용처 없음 |
